...
def load_data(path):
    ...
    test_nodes = np.array([id_map[n] for n in G.nodes() if G.nodes[n]['val']], dtype=np.int32)
    ...
    test_edges = [(e[0], e[1]) for e in edges if (e[0], e[1]) not in train_edges]
    
    edges = np.array(edges, dtype=np.int32)
    train_edges = np.array(train_edges, dtype=np.int32)
    test_edges = np.array(test_edges, dtype=np.int32)
    print(test_edges.shape, test_nodes.shape, train_nodes.shape)
    ...
    #test_adj = _construct_adj(test_edges, (len(test_nodes), len(test_nodes)))

    train_feats = feats[train_nodes]
    test_feats = feats[test_nodes]
            
    print("Data loaded in ", (time.time() - start)/60, "minutes.")
    return num_nodes, train_adj, train_feats, labels, train_nodes
